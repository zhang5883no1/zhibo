...
$(function() {
	...
	$.ajax( {
		...
		success : function(response) {
			...
			uSinaEmotionsHt.put('[pt掌声]', '/images/1.gif');
            uSinaEmotionsHt.put('[pt送鲜花]', '/images/2.gif');
            uSinaEmotionsHt.put('[pt顶一个]', '/images/3.gif');
            uSinaEmotionsHt.put('[pt赞一个]', '/images/4.gif');
            uSinaEmotionsHt.put('[pt给力]', '/images/5.gif');
            uSinaEmotionsHt.put('[pt棒！]', '/images/6.gif');
		}
	});
});
...
(function($){
	$.fn.SinaEmotion = function(target){
		...
		$(this).click(function(event){
			...
			var eTop = $("#topicinput").height() + 10;
			var eLeft = target.offset().left ;
			
			if($('#emotions .categorys')[0]){
				$('#emotions').css({left: eLeft});
				$('#emotions').toggle();
				return;
			}
			$('body').append('<div id="emotions"></div>');
			$('#emotions').css({bottom: eTop, left: eLeft});
