   ...
   $(function(){ 
	   ...
	   $.ajax({
			...
			success:function(res){
		        ...
		        var ln=getCookie("_Room1_loginName");
		        var lp=getCookie("_Room1_loginPwd");
		        if(ln!=""&&lp!=""){
		        	doLogin(ln,lp);
		        }else{
		        	doRegVisiter();
	   ...
	   $("#out").off().on().click(function(){
		   setCookie("_Room1_loginName","");
		   setCookie("_Room1_loginPwd","");
		   location.reload();
	   });
	   
	   $(".desktop").click(function(){
		   window.open("/download.jsp?filename="+encodeURI(encodeURI("凯旋直播间.url")));
	   });
   });
   ...
   function invokeGetInfo(){
	   $.ajax({
			...
			success:function(res){
				...
				if(topChats!=null&&topChats!=$("#marquee p").html()){
					$("#marquee p").html(topChats.content);
				}
   ...
   function putInfo(chat){
	   ...
	   if(chat.type==0||chat.type==10||chat.type==11){
		   var h=[];
		   if(chat.type==10){
			   h[h.length]='<div class="talk xun"><span class="time">'+ds[0]+':'+ds[1]+'</span><span><img title="皇冠" src="images/icon'+(7-parseInt(chat.faceImg))+'.png">';
			   h[h.length]='<div class="person_name"><a class="name" href="javascript:showName(\''+chat.userName+'\');">'+chat.userName+'</a></div></span>';
			   h[h.length]='<i class="speak">说：</i><div class="talk_hua xun_talk"><p>'+content+'</p></div></div>';
		   }else if(chat.type==11){
			   h[h.length]='<div class="talk guan"><span class="time">'+ds[0]+':'+ds[1]+'</span><span><img title="皇冠" src="images/icon'+(7-parseInt(chat.faceImg))+'.png">';
			   h[h.length]='<div class="person_name"><a class="name" href="javascript:showName(\''+chat.userName+'\');">'+chat.userName+'</a></div></span>';
			   h[h.length]='<i class="speak">说：</i><div class="talk_hua guan_talk"><p>'+content+'</p></div></div>';
		   }else{
			   h[h.length]='<div class="talk"><span class="time">'+ds[0]+':'+ds[1]+'</span><span><img title="皇冠" src="images/icon'+(7-parseInt(chat.faceImg))+'.png">';
			   h[h.length]='<div class="person_name"><a class="name" href="javascript:showName(\''+chat.userName+'\');">'+chat.userName+'</a></div></span>';
   ...
   function addValidChats(chat){
	   var ds=chat.date.split(" ")[1].split(":");
	   var h=[];
	   h[h.length]='<div id="valid_info_'+chat.id+'"><p><span class="hours">'+ds[0]+':'+ds[1]+'</span><img alt="铂金" src="images/icon'+(7-parseInt(chat.faceImg))+'.png">';
   ...
   function initAction(){
       ...
       $("#_info_level").attr("src","images/icon"+(7-parseInt(customer.level))+".png");
   }
   function showImg(){
	   if(parseInt(customer.level)>0){
